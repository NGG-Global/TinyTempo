...
show the same seventeen scenes over and over. No registry entry was added: the
...
before this level: `floor((level - 1) / VIGNETTES.length)`. With seventeen acts,
levels 1 to 17 are lap 0, 18 to 34 are lap 1, and so on. `PlayScene` passes it to
...
| --- | --- | --- | --- | --- |
| Bug & shoe (level 3, 20, 37…) | Plum bug, slate sneaker, coral tab | Ladybird, navy sneaker, mustard tab | Green beetle, burgundy sneaker, sky tab | `src/vignettes/bugLooks.ts` |
| Bicep curl (level 6, 23, 40…) | The coach: quiff, moustache, teal singlet, bolt | The sprinter: bun, plum singlet, star | The veteran: bald, grey beard, amber singlet, stripes | `src/vignettes/curlLooks.ts` |
| Scissors & paper (level 9, 26, 43…) | Star, heart, angel | Butterfly, fir tree, tulip | Star, heart, angel again | `PAPER_SHAPE_SETS` in `src/vignettes/paperMotion.ts` |
...
`tests/curl.test.ts`, `tests/paper.test.ts`). In DEV, `?debug&level=20`,
`?debug&level=23` and `?debug&level=26` open the lap 1 looks directly. The lap
