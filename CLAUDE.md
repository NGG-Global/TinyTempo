...
edit. On the map every finale in the window is a **stage**, not a flagged stop: a plaza the
road widens into, bunting in the treatment's pennants, a crown of three star seats over a
larger puck, and a plate naming the act (`definitionForLap`), locked and previewed ones faded
with a padlock badge (`finaleStopLook`, `ui/roadLayout.ts`). **A finale takes more road than a
stop** — `ROAD.finaleRoom`, because every area's gate stands half a step under its first level,
