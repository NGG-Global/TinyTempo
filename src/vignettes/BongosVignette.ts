...
export class BongosVignette extends HouseholdVignette {
  public constructor(scene: Phaser.Scene) { super(scene, 0xf0e3c9, 0xf8b77c); }
  ...
  protected draw(now: number, ending: number): void {
    ...
    this.drum(-112, -8, 110, 139, 0xbc643e);
    this.drum(113, 5, 124, 147, 0xb57b42);
    ...
    for (let side = 0; side < 2; side++) {
      ...
      g.lineStyle(43, 0x267673).lineBetween(hx + (side ? 1 : -1) * 37, -212, hx, hy - 52);
      g.lineStyle(8, 0x66a79a).lineBetween(hx + (side ? 1 : -1) * 35 - 10, -206, hx - 10, hy - 62);
      palm(g, hx, hy, side, side ? 0xd69268 : 0xe3a379);
      if (finale && this.successful && !this.still && ending < 1.6) {
        const p = (ending + side * 0.2) % 0.6 / 0.6;
        note(g, x + (side ? 1 : -1) * (52 + p * 42), -60 - p * 91, 0x397970, 1 - p);
      }
    }
  ...
  private drum(x: number, y: number, r: number, height: number, colour: number): void {
    const g = this.art;
    shape(g, [x - r, y, x + r, y, x + r * 0.76, y + height, x - r * 0.76, y + height], colour, 0x65442f, 3);
    for (let i = 0; i < 10; i++) {
      const u = -1 + i / 5, v = u + 0.2;
      shape(g, [x + r * u, y + 12, x + r * v, y + 12, x + r * v * 0.76, y + height - 8, x + r * u * 0.76, y + height - 8],
        i % 2 ? 0xe2a257 : 0x8a4e32, colour, 0);
